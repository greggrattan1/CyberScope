...
GO
DROP PROCEDURE IF EXISTS [dbo].[insFISMA_CIO_Metric_Staging_FY23_Q1]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[insFISMA_CIO_Metric_Staging_FY23_Q1]
(
...
-- EXEC [FCEBCyberScorecard].[dbo].[insFISMA_CIO_Metric_Staging_FY23_Q1] 'Gregory L. Grattan'

/*----------------------------------------------------------------------------------------------------
Creation Date = 07/06/2023 By Gregory L. Grattan

Description: Populates the [FCEBCyberScorecard].[Cyber].[FISMA_CIO_Metric_Staging] For FY23 Q1
...
FROM [FCEBCyberScorecard].[cyber].[FISMA_CIO_Metric_FirstFour_Staging] ff WITH(NOLOCK)--POWERSHELL
------------------------------------------------------------------------
--UNPIVOT DATA
...
,Agency_ID = a.CyberScopePK_Agency
,Question_ID = CONVERT(VARCHAR(20), @spReportingYear  + '_' + '01_' + f.IdentifierText) --Q's From FY23Q1
...
UNION ALL 
 SELECT 
 Reporting_Year = LEFT(RIGHT(ao.[Reporting Period], 7), 4)
,Reporting_Quarter = RIGHT(ao.[Reporting Period], 1) 
,Agency_ID = ISNULL(sa.CyberScopePK_Agency, '')
,Question_ID = CONVERT(VARCHAR(20), @spReportingYear  + '_' + '01_' + REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(ao.[Question Number], '.', ''), 'a', '1'), 'b', '2'), 'c', '3'), 'd', '4'), 'iii', '03'), 'ii', '02'), 'i', '01')) --Q's From FY23Q1
,Identifier_Text = ao.[Question Number]
,Answer = ao.Answer
,Last_Modified_Date = GETDATE()
,Last_Modified_By = @spLastModifiedBy
